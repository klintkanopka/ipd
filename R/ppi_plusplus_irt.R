...
#'
#' @param w_lo (ndarray, optional): Sample weights for the labeled data set
#' for observed responses.
#' Defaults to a vector of ones.
#'
#' @param w_lp (ndarray, optional): Sample weights for the labeled data set
...
ppi_plusplus_irt_est <- function(
  ...
  rectified_irt_loss <- function(theta) {
  ...
  rectified_irt_grad <- function(theta) {
    lhat_curr /
      N *
      t(X_u) %*% (w_u * (plogis(X_u %*% theta) - f_u)) -

      lhat_curr / n * t(X_l) %*% (w_lp * (plogis(X_l %*% theta) - f_l)) +

      1 / n * t(X_l) %*% (w_lo * (plogis(X_l %*% theta) - Y_l))
  }

  est <- optim(
    par = theta,
    fn = rectified_irt_loss,

    gr = rectified_irt_grad,
    method = "L-BFGS-B",
    control = list(factr = opts$factr)
  )$par

  if (is.null(lhat)) {
    stats <- irt_get_stats(est, X_l, Y_l, f_l, X_u, f_u, w_lo, w_lp, w_u)
...
#--- PPI++ IRT REGRESSION - INFERENCE -------------------------------------

#' PPI++ IRT via Logistic Regression
#'
#' @description
#' Helper function for PPI++ IRT analysis regression
#'
#' @details
#' PPI++: Efficient Prediction Powered Inference (Angelopoulos et al., 2023)
#' <https://arxiv.org/abs/2311.01453>
...
#'
#' @param w_lo (ndarray, optional): Sample weights for the labeled data set.
#' Defaults to a vector of ones. Observed responses.
#'
#' @param w_lp (ndarray, optional): Sample weights for the labeled data set.
#' Defaults to a vector of ones. Predicted responses
#'
#' @param w_u (ndarray, optional): Sample weights for the unlabeled
#' data set. Defaults to a vector of ones.
#'
...
ppi_plusplus_irt <- function(
  ...
  w_lo <- if (is.null(w_lo)) rep(1, n) else w_lo / sum(w_lo) * n
  w_lp <- if (is.null(w_lp)) rep(1, n) else w_lp / sum(w_lp) * n
